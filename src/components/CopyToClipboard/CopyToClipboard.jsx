import { useEffect, useState } from "react";
import Styles from "./styles.module.css";
import PropTypes from "prop-types";

CopyToClipbpard.propTypes = {
  resultValue: PropTypes.string,
};

export default function CopyToClipbpard({ resultValue }) {
  const [copiedToClipboard, setCopiedToClipboard] = useState(false);

  function handleOnClick() {
    navigator.clipboard.writeText(resultValue);
    if (!copiedToClipboard) setCopiedToClipboard(true);
  }

  useEffect(() => {
    if (copiedToClipboard) {
      setTimeout(() => {
        setCopiedToClipboard(false);
      }, 1000);
    }
  });

  return (
    <div className={Styles.calculator__clipboardWrapper}>
      <button
        role="button"
        title="Copy result to clipboard"
        onClick={handleOnClick}
      >
        <svg
          className={Styles.calculator__clipboard}
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
          fill="currentColor"
          viewBox="0 0 16 16"
          tabIndex="0"
        >
          <path d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1v-1z" />
          <path d="M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5h3zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3z" />
        </svg>
      </button>
      {copiedToClipboard && (
        <span className={Styles.calculator__clipboardNotification}>
          Copied!
        </span>
      )}
    </div>
  );
}
